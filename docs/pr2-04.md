...
add a deterministic token-native chat format + assistant-only loss masking + packing/truncation to produce fixed-size training batches for sft, using <|eot|> as the only stop boundary.
...
rules:
	1.	if serialized token ids length > S, left-truncate by dropping whole oldest turns (drop <|usr|>...<|eot|> + <|asst|>...<|eot|> pairs) until it fits.
...
tests must verify:
	...
	•	given a chat with no system message, serialization starts with <|sys|> and ends with <|eot|> for each turn.
